...
All the input tables were provides [here](./ExampleTables/). We provide a subset of each input table, as a reference for you to set your input format accordingly.
...
## Figure 3.
* step 1: group spots. In this case, we have 2 replicates, and group spots for each rep.
```{r, eval=FALSE}
...
![](./Images/spotsGrouped.png)

* step 2: generate the cnr file by running [cnvkit](https://www.youtube.com/watch?v=JGOIXYoLG6w&t=747s).
...
* step 5: sort cdt file. In our case, there are 2 replicates. The sorted cdt file is saved to your working dir.
```{r eval=FALSE}
...
```
CNVs_sorted.cdt is generated. 
![](./Images/CNVs_sorted.png)
the CNVs_sorted.cdt is the spot CNVs data matrix, which can be visualized either in JavaTreeview to make Figure3 or in R as following:
```{r}
...
## Figure 4
Generate SNPMajorAlleleCount.csv for each rep (ex: patient6 rep1); as an example, we only include 2 spots here.
```{r eval=FALSE}
...
```
The SNPallMajorAlleleCount.csv and SNPMajorAlleleCount.csv files are saved in your working dir.
