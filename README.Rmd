...
you need to prepare 4 input files in your working dir. The 4 inputs are filtered_feature_bc.csv, Graph-Based.csv, tissue_positions_list.csv, annotate.csv. Next, you can run the docker image following the example below. <br />
...
### StereoSeq Platform
First, run the following code to generate mimic input files similar to 10X platform.
...
Second, repeat the same code as in 10X Platform after tumor, non-tumor clusters annotation.

Example 1: group spots by number of genes when bulk tumor CNVs is available.

```docker run -v <your_local_dir>:/home/stmut limin321/stmutcnv_arm64:0.0.1 bash /usr/local/bin/stmutcnv.sh 
...
Example 2: group spots by number of reads without bulk tumor CNVs available <br />
`
docker run -v <your_local_dir>:/home/stmut limin321/stmutcnv_arm64:0.0.1 bash /usr/local/bin/stmutcnv.sh 
    --filteredFeatureCSV ./filtered_feature_bc.csv 
    --clusterCSV ./Graph-Based.csv 
